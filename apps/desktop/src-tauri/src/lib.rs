...
pub struct RenderRequest {
	...
	/// Absolute output path, e.g. `C:\Users\...\video.mp4`.
	pub out_path: String,
	pub width: u32,
	pub height: u32,
	pub fps: u32,
	...
	pub codec: String,
	/// kbps (e.g. 8000 = 8 Mbps).
	#[serde(default = "default_bitrate")]
	pub bitrate_kbps: u32,
	/// Whether the timeline contains any audio tracks. When true, EVERY
	/// clip must contain an audio stream (concat filter requirement).
...
/// Shared render state managed by Tauri.
#[derive(Default)]
...
/// Detect an available hardware encoder by test-encoding one tiny frame.
/// Falls back to a software encoder when nothing matches.
...
// ──────────────────────────────────────────────
// Render implementation (blocking, runs on a worker thread)
// ──────────────────────────────────────────────
...
fn run_render(
	...
	// Map outputs: single clip keeps original labels, multi-clip uses concat out.
	...
	*pid_slot.lock().unwrap() = child.id();
	...
	// Drain stderr on its own thread so it never backpressures the pipe.
...
pub fn probe_hw_encoder(app: AppHandle, codec: String) -> String {
	let ffmpeg = ffmpeg_path(&app);
	detect_hw_encoder(&ffmpeg, &codec)
}
...
/// Render a list of clips into one file using the bundled FFmpeg sidecar.
/// Emits `render-progress` events while running. Heavy work happens on a
/// blocking worker thread so the UI stays responsive.
#[tauri::command]
...
/// Resolve the bundled sidecar binary. Falls back to PATH ffmpeg (dev mode).
fn ffmpeg_path(app: &AppHandle) -> String {
	app.path()
		.resource_dir()
		.ok()
		.map(|d| {
			d.join("binaries/ffmpeg")
...
/// Cross-process kill (best-effort).
fn kill_pid(pid: u32) {
